...
function Casino(casinochan) {
    ...
    this.showHelp = function(src, data) {
        if (data === "*") {
            ...
            casinobot.sendMessage(src, "Use the coins earned in #Casino to bet in #Pokemon Race!", channel)
            sys.sendMessage(src, border, casinochan);
            sys.sendMessage(src, "", casinochan);
            return;
        }
        ...
        switch (name) {
            ...
            case "slots":
                ...
                break;
            default:
                casinobot.sendMessage(src, "No such command!", casinochan);
                return;
        }
        sys.sendMessage(src, "", casinochan);
        for (var x in out) {
    ...
    this.handleCommand = function(src, message, channel) {
        ...
        try {
            ...
            if (e !== "No valid command") {
                ...
                if (sys.id("Kase") !== undefined) {
                    sys.sendMessage(sys.id("Kase"), "Error on Casino command" + (e.lineNumber ? " on line " + e.lineNumber : "") + ": " + e + " [" + sys.name(src) + " typed /" + message + "]", casinochan);
                }
                return true;
            }
        }
