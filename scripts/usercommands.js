exports.handleCommand = function(src, command, commandData, tar, channel) {
    ...
    if ((command == "me") && !SESSION.channels(channel).muteall) {
        ...
        if (SESSION.users(src).smute.active) {
            sys.playerIds().forEach(function(id) {
                if (sys.loggedIn(id) && SESSION.users(id).smute.active && sys.isInChannel(src, channel)) {
                    ...
                    sys.sendHtmlMessage(id, "<font color='" + colour + "'><timestamp/> *** <b>" + utilities.html_escape(sys.name(src)) + "</b></i></font> " + commandData + "</i> ***", channel);
                }
            });
            sys.stopEvent();
        ...
        if (command == "me") {
            var colour = script.getColor(src);
            sendChanHtmlAll("<font color='" + colour + "'><timestamp/> *** <b>" + utilities.html_escape(sys.name(src)) + "</b></font> " + messagetosend + " </i> ***", channel);
        }
        script.afterChatMessage(src, '/' + command + ' ' + commandData, channel);
        return;
    }
...
exports.help =
    [
        ...
        "/contributors: Lists contributors to 7 Seas.",
